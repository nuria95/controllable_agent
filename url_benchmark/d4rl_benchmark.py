...
class D4RLReplayBufferBuilder:
    ...
    def filter_dataset_by_episode_length(self, dataset: tp.Any, minimum_episode_length: tp.Optional[int]):
        ...
        diff_len = dataset['observations'].shape[0] - \
            len(episode_lengths_expanded)
        # there is no guarantee that last step in the data step is last step in an episode
        assert diff_len >= 0
        episode_lengths_expanded = np.concatenate((episode_lengths_expanded, np.zeros(
            diff_len, dtype=int)))  # ignore last steps if they do not belong to an episode
    ...
    def prepare_replay_buffer_d4rl(self, env: EnvWrapper, meta: MetaDict, cfg: tp.Any) -> ReplayBuffer:
        ...
        dataset = self.filter_dataset_by_episode_length(
            dataset, cfg.d4rl_config.minimum_episode_length)
        ...
        is_ignore_terminals = cfg.d4rl_config and (
            cfg.d4rl_config.ignore_terminals)
        terminals = np.zeros_like(
            dataset['terminals']) if is_ignore_terminals else dataset['terminals']
        ...
        if not cfg.d4rl_config or cfg.d4rl_config.minimum_episode_length is None:
            ...
            assert (episode_lengths <
                    cfg.d4rl_config.minimum_episode_length).sum() == 0
        replay_storage = ReplayBuffer(max_episodes=len(
            end_indices), discount=cfg.discount, future=cfg.future, max_episode_length=max_episode_length)
        first = True
