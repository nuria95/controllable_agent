...
final_hyperparams = {'hopper': [
    ...
    [(True, 0.3, None, 100, False, 1., None), ourdarkred],
    ...
    [(True, 0.3, True, 100, True, 0., None), ourorange],
      [(True, 0.3, True, 100, False, 1., None), ourdarkred],
...
dir_figs = '/home/nuria/phd/controllable_agent/figs/exp_extra_baselines2'
paths = [f'{BASE_PATH}/quadruped',
         f'{BASE_PATH}/quadruped_zprobab2',  # rerunning longer
         ...
         f'{BASE_PATH}/maze3_policy',
         f'{BASE_PATH}/cheetah2_policy',
...
TASK_PATHS = [
    ...
    [paths[3], paths[4], paths[5], paths[15], paths[20]],  # maze
    [paths[6], paths[7], paths[8], paths[16]],  # walker
    [paths[9], paths[10], paths[17], ],  # hopper
    [paths[11], paths[12], paths[13], paths[21], paths[18] ],  # cheetah
]
yaxis_cut = True
        ...
        for env_task, groups in grouped_data.items():
            ...
            for group_key in groups.keys():
                ...
                if group_key in list(map(lambda x: x[0], final_hyperparams[env])) or group_key in final_hyperparams_random:
                    ...
                    if env=='maze' and group_key == (True, 0.3, True, 100, False, 1.0, None) :
                        try:
                            rews_seeds = [rew for rew in rews_seeds if len(rew) > 14]
                        except:
                            breakpoint()
