...
# group_key = (uncertainty, mix_ratio, add_trunk, update_z_every, sampling, update_z_proba, expl_strategy, funcertainty)
final_hyperparams = {'hopper': [

    [(True, 0.3, None, 100, True, 1., None, False), ourorange],
    [(True, 0.3, None, 100, True, 1., None, True), ourblue],
],

    'maze': [
    [(True, 0.3, True, 100, True, 1., None, False), ourorange],
    [(True, 0.3, True, 100, True, 1., None, True), ourblue],
...
dir_figs = '/home/nuria/phd/controllable_agent/figs/funcertainty'
paths = [f'{BASE_PATH}/quadruped',
         f'{BASE_PATH}/quadruped_funcertainty',

         f'{BASE_PATH}/maze3',
         f'{BASE_PATH}/maze3_funcertainty',

         f'{BASE_PATH}/walker',
         f'{BASE_PATH}/walker_funcertainty',

         f'{BASE_PATH}/hopper',
         f'{BASE_PATH}/hopper_funcertainty',

         f'{BASE_PATH}/cheetah2',
         f'{BASE_PATH}/cheetah2_funcertainty',
...
for TASK_PATH in TASK_PATHS:
    ...
    for exp in files:
        ...
        f_uncertainty = config.get("agent").get("f_uncertainty", False)

        # Use (uncertainty, mix_ratio) as the group key
        group_key = (uncertainty, mix_ratio, add_trunk, update_z_every,
                     sampling, update_z_proba, expl_strategy, f_uncertainty)
        ...
        for env_task, groups in grouped_data.items():
            ...
            for group_key in groups.keys():
                ...
                if group_key in list(map(lambda x: x[0], final_hyperparams[env])) or group_key in final_hyperparams_random:
                    ...
                    if len(rews_seeds) < 7:
                        print(
                            f'Removed too many ALL TASKS runs! {len(rews_seeds)}, {env}')
                    ...
                    if len(
                            rews_seeds) < 7:
                        print(
                            f'Removed too many RND runs! {len(rews_seeds)}, {env}')
                        continue
                    min_len = min([len(rew) for rew in rews_seeds])
                    rews_seeds = [rew[:min_len] for rew in rews_seeds]
            ...
            # Continuous lines, grey color
            axs[plt_num].grid(True, linestyle='-', color='grey')
        ...
        for group_key in groups.keys():
            if group_key in list(map(lambda x: x[0], final_hyperparams[env])) or group_key in final_hyperparams_random:
                ...
                if len(
                        rews_seeds) < 7:
                    print(
                        f'Removed too many RND AVG runs! {len(rews_seeds)}, {env}')
                min_len = min([len(rew) for rew in rews_seeds])
                rews_seeds = [rew[:min_len] for rew in rews_seeds]
